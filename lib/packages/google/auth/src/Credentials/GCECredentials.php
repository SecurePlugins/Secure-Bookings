...
use Automattic\WooCommerce\Bookings\Vendor\Google\Auth\IamSignerTrait;
            ...
            try {
                ...
                $resp = $httpHandler(
                    new Request(
                        'GET',
                        $checkUri,
                        [self::FLAVOR_HEADER => 'Automattic\WooCommerce\Bookings\Vendor\Google']
                    ),
                    ['timeout' => self::COMPUTE_PING_CONNECTION_TIMEOUT_S]
                );

                return $resp->getHeaderLine(self::FLAVOR_HEADER) == 'Automattic\WooCommerce\Bookings\Vendor\Google';
